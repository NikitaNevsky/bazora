...
class OptovikMobileSupabaseUser extends BaseAuthUser {
  OptovikMobileSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> optovikMobileSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = OptovikMobileSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
